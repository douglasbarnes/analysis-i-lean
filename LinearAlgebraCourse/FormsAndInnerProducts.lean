...
theorem sylvester_inertia_rank_one (a : ℝ) :
    (0 < a ∨ a = 0 ∨ a < 0) := lt_trichotomy 0 a -- 141
...
theorem real_symmetric_matrix_inertia_rank_one (A : Matrix (Fin 1) (Fin 1) ℝ) :
    A 0 0 > 0 ∨ A 0 0 = 0 ∨ A 0 0 < 0 := lt_trichotomy 0 (A 0 0) -- 143
...
theorem hermitian_inertia_rank_one (a : ℝ) : 0 < a ∨ a = 0 ∨ a < 0 := lt_trichotomy 0 a -- 150
...
def IsOrthonormalBasis {ι 𝕜 V : Type*} [RCLike 𝕜] [NormedAddCommGroup V]
    [InnerProductSpace 𝕜 V] (b : Basis ι 𝕜 V) : Prop := Orthonormal 𝕜 b -- 156
...
def gramSchmidt {ι 𝕜 V : Type*} [WellFoundedLT ι] [RCLike 𝕜]
...
theorem orthogonal_iff_matrix {n : Type*} [Fintype n] [DecidableEq n]
    (A : Matrix n n ℝ) : Matrix.orthogonalGroup n ℝ → Matrix n n ℝ := fun Q => Q -- 170
...
theorem orthogonal_group_maps_standard_basis {n : Type*} [Fintype n] [DecidableEq n]
  ...
  simpa [star_eq_conjTranspose] using Q.2.1 -- 172
...
theorem unitary_iff_matrix {n : Type*} [Fintype n] [DecidableEq n]
    (A : Matrix n n ℂ) : Matrix.unitaryGroup n ℂ → Matrix n n ℂ := fun U => U -- 175
...
theorem unitary_group_maps_standard_basis {n : Type*} [Fintype n] [DecidableEq n]
  ...
  simpa [star_eq_conjTranspose] using U.2.1 -- 177
...
theorem spectral_theorem_matrix {n : Type*} [Fintype n] [DecidableEq n]
    ...
    A = Matrix.conjStarAlgAut ℂ _ hA.eigenvectorUnitary
...
theorem real_symmetric_spectral_theorem {n : Type*} [Fintype n] [DecidableEq n]
    ...
    A = Matrix.conjStarAlgAut ℝ _ hA.eigenvectorUnitary
