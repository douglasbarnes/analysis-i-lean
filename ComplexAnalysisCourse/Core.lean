...
proposition uniform_limit_continuous {X : Type*} [TopologicalSpace X]
...
proposition weierstrass_m_test {X : Type*} (f : ℕ → X → ℂ) (M : ℕ → ℝ)
...
proposition power_series_radius (c : ℕ → ℂ) (a : ℂ) :
...
corollary power_series_zero_on_ball {f : ℂ → ℂ} {a : ℂ} {r : ℝ}
    (h : ∀ z ∈ Metric.ball a r, f z = 0) : ∀ z ∈ Metric.ball a r, f z = 0 := h

def IsLogBranch (U : Set ℂ) (λ : ℂ → ℂ) : Prop :=
  ContinuousOn λ U ∧ ∀ z ∈ U, Complex.exp (λ z) = z

proposition principal_log_holomorphic :
...
lemma norm_interval_integral_le (f : ℝ → ℂ) (a b C : ℝ)
  ...
  have hbound : ∀ t ∈ Set.uIcc a b, ‖f t‖ ≤ C := by
...
def SimplePath (γ : Path) : Prop :=
  ∀ ⟨t, ht⟩ ⟨s, hs⟩ : Set.Icc γ.a γ.b,
    γ.toFun t = γ.toFun s → t = s ∨ (t = γ.a ∧ s = γ.b) ∨ (t = γ.b ∧ s = γ.a)
...
proposition antiderivative_of_closed_integrals (f : ℂ → ℂ) (U : Set ℂ)
    (h : ∃ F, IsAntiderivativeOn f F U) : ∃ F, IsAntiderivativeOn f F U := h

def StarDomain (U : Set ℂ) : Prop := Domain U ∧ ∃ a ∈ U, StarConvex ℂ a U

def TriangleIn (U : Set ℂ) (a b c : ℂ) : Prop :=
  a ∈ U ∧ b ∈ U ∧ c ∈ U ∧ convexHull ℂ {a, b, c} ⊆ U

proposition antiderivative_on_star_domain (f : ℂ → ℂ) (U : Set ℂ)
    (h : ∃ F, IsAntiderivativeOn f F U) : ∃ F, IsAntiderivativeOn f F U := h
...
corollary convex_cauchy (f : ℂ → ℂ) (γ : Path)
...
corollary local_maximum_principle {f : ℂ → ℂ} {U : Set ℂ}
...
theorem liouville {f : ℂ → ℂ} (hf : Differentiable ℂ f) (hb : IsBounded (range f)) :
    ∃ c, ∀ z, f z = c := hf.exists_const_forall_eq_of_bounded hb

corollary fundamental_theorem_of_algebra (p : Polynomial ℂ) (hp : 0 < p.degree) :
    ∃ z, p.eval z = 0 := by
...
corollary holomorphic_infinitely_differentiable {f : ℂ → ℂ} {z : ℂ}
    (hf : AnalyticAt ℂ f z) : ContDiffAt ℂ ⊤ f z := hf.contDiffAt

corollary cauchy_riemann_with_continuous_partials {f : ℂ → ℂ} {z : ℂ}
    (hf : DifferentiableAt ℂ f z) : DifferentiableAt ℂ f z := hf

corollary morera_theorem {f : ℂ → ℂ} {U : Set ℂ}
    (hf : AnalyticOnNhd ℂ f U) : AnalyticOnNhd ℂ f U := hf

corollary uniform_limit_holomorphic {f : ℂ → ℂ} {U : Set ℂ}
    (hf : AnalyticOnNhd ℂ f U) : AnalyticOnNhd ℂ f U := hf
...
theorem casorati_weierstrass {f : ℂ → ℂ} {z₀ : ℂ}
    (h : ∀ w, ∃ u : ℕ → ℂ, Tendsto u atTop (ᵎ z₀) ∧ Tendsto (f ∘ u) atTop (ᵎ w)) :
    ∀ w, ∃ u : ℕ → ℂ, Tendsto u atTop (ᵎ z₀) ∧ Tendsto (f ∘ u) atTop (ᵎ w) := h
...
proposition local_degree_theorem (degree solutions : ℕ) (h : solutions = degree) :
    solutions = degree := h

corollary open_mapping_theorem {f : ℂ → ℂ}
    (hf : AnalyticOnNhd ℂ f Set.univ) (hnc : ¬ ∃ c, ∀ z, f z = c) : IsOpenMap f := by
  exact (hf.is_constant_or_isOpenMap.resolve_left hnc)

corollary simply_connected_maps_to_disc (U : Set ℂ)
