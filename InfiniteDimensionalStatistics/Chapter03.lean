...
results, exact VC complement permanence, and measurable continuous-mapping and
Slutsky theorems.  The product-geometry, entropy-method, empirical-entropy,
canonical U-statistic, entropy-growth, Gaussian-bridge, Donsker, local-class,
and outer-law interfaces used through Section 3.7 are also present.  Deep
entropy, VC and Donsker theorem proofs are added only when they have genuine Lean
proofs; no assumption or placeholder is used to simulate completion.
-/
