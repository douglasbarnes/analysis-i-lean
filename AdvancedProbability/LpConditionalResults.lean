...
theorem condExp_norm_rpow_le {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  · have hl :=
    ...
    filter_upwards [norm_condExp_le (μ := μ) (m := m) (f := f), hj] with a ha hb
...
theorem memLp_condExp_ofReal {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  · exact ((Real.continuous_rpow_const hp_pos.le).comp_stronglyMeasurable
      ((stronglyMeasurable_condExp (μ := μ) (m := m) (f := f)).mono hm).norm).aestronglyMeasurable
  · filter_upwards with a
    positivity
...
theorem eLpNorm_condExp_le_eLpNorm_ofReal {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  · have hce := memLp_condExp_ofReal hm hp hf
    ...
    exact Real.rpow_le_rpow (integral_nonneg fun _ ↦ by positivity)
      (integral_norm_condExp_rpow_le hm hp hf_pow) (by positivity)
