...
theorem condExp_norm_rpow_le {Ω : Type u} [m₀ : MeasurableSpace Ω]
    {m : MeasurableSpace Ω} {μ : Measure Ω} {p : ℝ} (hp : 1 ≤ p)
    {f : Ω → ℝ} (hfint : Integrable (fun x ↦ ‖f x‖ ^ p) μ) :
    (fun x ↦ ‖μ[f | m] x‖ ^ p) ≤ᵐ[μ] μ[(fun x ↦ ‖f x‖ ^ p) | m] := by
  have hp' : 0 < p := by linarith
  by_cases! hm : ¬ m ≤ m₀
  · simp [condExp_of_not_le hm, Real.zero_rpow hp'.ne.symm]
  by_cases! hμm : ¬ SigmaFinite (μ.trim hm)
  · simp [condExp_of_not_sigmaFinite hm hμm, Real.zero_rpow hp'.ne.symm]
  ...
  · simp only [condExp_of_not_integrable hf_int, Pi.zero_apply, norm_zero,
    ...
    positivity
  have hl := (Real.continuous_rpow_const hp'.le).lowerSemicontinuous.lowerSemicontinuousOn (Set.Ici 0)
...
theorem integral_norm_condExp_rpow_le {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  _ ≤ ∫ x, μ[(fun x ↦ ‖f x‖ ^ p) | m] x ∂μ := by
    refine integral_mono_of_nonneg ?_ integrable_condExp (condExp_norm_rpow_le hp hf)
    filter_upwards with a
    positivity
  _ ≤ _ := by
...
theorem memLp_condExp_ofReal {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  have hp0 : ENNReal.ofReal p ≠ 0 := by simp [hp_pos.ne']
  have hpt : ENNReal.ofReal p ≠ ⊤ := ENNReal.ofReal_ne_top
  rw [← integrable_norm_rpow_iff integrable_condExp.1 hp0.symm hpt]
  rw [ENNReal.toReal_ofReal hp_pos.le]
  have hf_pow : Integrable (fun x ↦ ‖f x‖ ^ p) μ := by
    rw [← ENNReal.toReal_ofReal hp_pos.le]
    exact (integrable_norm_rpow_iff hf.1 hp0.symm hpt).2 hf
  refine Integrable.mono_nonneg integrable_condExp ?_ ?_ (condExp_norm_rpow_le hp hf_pow)
  · fun_prop
  · filter_upwards with a
    positivity
...
theorem eLpNorm_condExp_le_eLpNorm_ofReal {Ω : Type u} [m₀ : MeasurableSpace Ω]
  ...
  · have hce := memLp_condExp_ofReal (m := m) hp hf
    rw [← ofReal_lpNorm hf, ← ofReal_lpNorm hce]
    apply ENNReal.ofReal_le_ofReal
    have hp_pos : 0 < p := zero_lt_one.trans_le hp
    have hp0 : ENNReal.ofReal p ≠ 0 := by simp [hp_pos.ne']
    have hpt : ENNReal.ofReal p ≠ ⊤ := ENNReal.ofReal_ne_top
    rw [lpNorm_eq_integral_norm_rpow_toReal hp0.symm hpt hf.1,
      lpNorm_eq_integral_norm_rpow_toReal hp0.symm hpt hce.1,
      ENNReal.toReal_ofReal hp_pos.le]
    gcongr
    exact integral_norm_condExp_rpow_le hp <| by
      rw [← ENNReal.toReal_ofReal hp_pos.le]
      exact (integrable_norm_rpow_iff hf.1 hp0.symm hpt).2 hf
