...
abbrev SymmetricDifference {α : Type*} (A B : Set α) := A ∆ B
...
abbrev PartialOrderRelation {α : Type*} (r : Relation α) := IsRefl α r ∧ Antisymm r ∧ _root_.Transitive r
...
abbrev Integers := ℤ
abbrev Rationals := ℚ
abbrev TotallyOrderedField (F : Type*) := LinearOrderedField F
