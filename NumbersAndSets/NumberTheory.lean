...
theorem division_algorithm (a : ℤ) {b : ℤ} (hb : 0 < b) :
    ∃ q r : ℤ, a = q * b + r ∧ 0 ≤ r ∧ r < b := by
  refine ⟨a / b, a % b, ?_, Int.emod_nonneg _ (ne_of_gt hb), Int.emod_lt_of_pos _ hb⟩
  simpa [mul_comm] using (Int.ediv_add_emod a b).symm
...
theorem bezout_identity (a b : ℕ) :
  ...
  refine ⟨(a : ℤ).gcdA b, (a : ℤ).gcdB b, ?_⟩
  simpa [mul_comm] using Int.gcd_eq_gcd_ab (a : ℤ) (b : ℤ)
...
theorem exists_prime_factor {n : ℕ} (hn : n ≠ 1) : n = 0 ∨ ∃ p, p.Prime ∧ p ∣ n := by
  by_cases h0 : n = 0
  · exact Or.inl h0
  · exact Or.inr (Nat.exists_prime_and_dvd hn)
...
theorem fermat_little {p a : ℕ} (hp : p.Prime) : Nat.ModEq p (a ^ p) a :=
  by
    exact_mod_cast Int.ModEq.pow_prime_eq_self hp (a : ℤ)

theorem euler_fermat {a m : ℕ} (h : Nat.Coprime a m) :
    Nat.ModEq m (a ^ Nat.totient m) 1 := Nat.ModEq.pow_totient h

theorem totient_prime {p : ℕ} (hp : p.Prime) : Nat.totient p = p - 1 := Nat.totient_prime hp
