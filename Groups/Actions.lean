...
theorem orbit_stabilizer_card (G : Type u) {X : Type v} [Group G] [Fintype G]
    [MulAction G X] (x : X) :
    Nat.card (MulAction.orbit G x) * Nat.card (MulAction.stabilizer G x) = Fintype.card G := by
...
def cayleyEmbedding (G : Type u) [Group G] : G ↪* Equiv.Perm G :=
  CayleyEmbedding G

/-- 093. Left multiplication on cosets is transitive. -/
theorem left_coset_action_transitive {G : Type u} [Group G] (H : Subgroup G) :
    ∀ x y : Quotient (QuotientGroup.leftRel H), ∃ g : G, g • x = y := by
  intro x y
  induction x using Quotient.inductionOn with
  | _ a =>
    induction y using Quotient.inductionOn with
    | _ b => exact ⟨b * a⁻¹, by simp [mul_assoc]⟩
...
theorem normal_conjugate_mem {G : Type u} [Group G] (K : Subgroup G) [K.Normal]
  ...
  simpa [conjugate] using K.Normal.conj_mem k hk g
...
theorem normal_iff_conjugacy_closed {G : Type u} [Group G] (K : Subgroup G) :
  ...
  constructor
  · intro h g k hk
    simpa [conjugate] using h.conj_mem k hk g
  · intro h
...
abbrev normalizerOf {G : Type u} [Group G] (H : Subgroup G) := H.normalizer
...
theorem stabilizer_smul {G : Type u} {X : Type v} [Group G] [MulAction G X]
  ...
  ext h
  simp [conjugateSubgroup, conjugate, mul_smul]

/-- 103. Cauchy's theorem. -/
theorem cauchy {G : Type u} [Group G] [Fintype G] {p : ℕ}
    (hp : Nat.Prime p) (hd : p ∣ Fintype.card G) : ∃ x : G, orderOf x = p := by
  exact exists_orderOf_eq_prime hp hd

/-- 104. Conjugating a permutation transports its cycle. -/
theorem conjugate_cycle {X : Type u} [DecidableEq X] (ρ : Equiv.Perm X) (l : List X) :
    ρ * Equiv.Perm.cycle l * ρ⁻¹ = Equiv.Perm.cycle (l.map ρ) := by
  exact Equiv.Perm.conj_cycle ρ l

/-- 105. Conjugation preserves cycle type. -/
theorem cycleType_conjugate {X : Type u} [Finite X] [DecidableEq X]
...
def ConjugacyClassSplits {X : Type u} [Fintype X] [DecidableEq X]
  ...
  Nat.card (ConjAct.orbit σ) * 2 = Nat.card (ConjAct.orbit (σ : Equiv.Perm X))
...
theorem index_two_class_splitting_criterion {G : Type u} [Group G] (H : Subgroup G)
  ...
  simp only [Subgroup.le_def, SetLike.mem_coe]
  constructor
  · intro h g hg
    apply h g
    simpa [mul_assoc] using hg x
  · intro h g hg
    apply h
    intro y hy
    simpa [Set.mem_singleton_iff.mp hy, mul_assoc] using hg

/-- 108. A 5-cycle generates a cyclic subgroup of order five. -/
theorem five_cycle_order {X : Type u} [DecidableEq X] (l : List X)
    (hl : l.Nodup) (hlen : l.length = 5) : orderOf (Equiv.Perm.cycle l) = 5 := by
  subst hlen
  exact orderOf_cycle l hl (by decide)
...
theorem quaternion_order : Nat.card QuaternionEight = 8 := by decide
